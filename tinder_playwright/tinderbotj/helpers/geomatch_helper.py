...
class GeomatchHelper:
    ...
    delay = 5000  # Playwright uses milliseconds
    ...
    def like(self)->bool:
        try:
            ...
            #    self.page.wait_for_selector(xpath, timeout=self.delay)
                ...
                # locate like button
            #    like_button = self.page.locator(xpath)
            ...
            #    self.page.wait_for_selector(xpath, timeout=self.delay)
             ...
             #   card = self.page.locator(xpath)

            #    card.drag_to(card, target_position={'x': 200, 'y': 0})
    ...
    def dislike(self):
        try:
            ...
            #    dislike_button = self.page.locator(xpath)
    ...
    def superlike(self):
        try:
            if 'profile' in self.page.url:
                ...
                superlike_button = self.page.locator(xpath)
                ...
                card = self.page.locator(xpath).first

                # Drag card upwards
                card.drag_to(card, target_position={'x': 0, 'y': -200})
    ...
    def get_name(self):
        ...
        try:
            ...
            if not name:
                ...
                element2 = self.page.locator(xpath2)
                name = element2.text_content()
    ...
    def is_verified(self):
        ...
        try:
            self.page.locator(xpath_badge).wait_for(state='attached', timeout=1000)
            return True
    ...
    def get_row_data(self):
        ...
        for row in rows:
            svg = row.locator(".//*[starts-with(@d, 'M')]").get_attribute('d')
            value = row.locator(".//div[2]").text_content()
    ...
    def get_bio_and_passions(self):
        ...
        # Bio
        try:
            bio = self.page.locator('div[class*="Px(16px) Py(12px) Us(t)"').text_content()
        ...
        # Looking for
        try:
            looking_for_el = self.page.locator('div[class="Px(16px) My(12px)"]>div[class="D(b)"]')
            looking_for = looking_for_el.locator('div[class="Typs(subheading-1) CenterAlign"]').text_content()
        ...
        try:
            sections = self.page.locator("div[class='Px(16px) Py(12px)']").all()
            for section in sections:
                headline = section.locator("h2").text_content().lower()

                if headline in infoItems.keys():
                    infoElements = section.locator("div[class^='Bdrs(100px)']").all()
                    for infoElement in infoElements:
                        infoItems[headline].append(infoElement.text_content())
                elif headline == 'my anthem':
                    song = section.locator("div[class$='C($c-ds-text-primary)']").text_content()
                    artist = section.locator("div[class$='C($c-ds-text-secondary)']").text_content()
                    anthem = {
                        "song": song,
                        "artist": artist
                    }
            ...
            #if ('Passions' in passions_el.locator("h2").text_content()):
            #    #print("Passions Text", passions_el.text_content())
            #    elements = passions_el.locator('div').locator('div').locator('div').all()
            #    for el in elements:
    ...
    def get_image_urls(self, quickload=True):
        ...
        for element in elements:
            # Get background-image CSS property using evaluate
            image_url = element.evaluate('el => window.getComputedStyle(el).backgroundImage')
        ...
        try:
            ...
            # wait for element to appear
            self.page.wait_for_selector(f'.{classname}', timeout=self.delay)

            image_btns = self.page.locator(f'.{classname}').all()
            ...
            try:
                element = self.page.locator("//div[@aria-label='Profile slider']").first
                image_url = element.evaluate('el => window.getComputedStyle(el).backgroundImage')
                if image_url and 'url(' in image_url:
                    image_url = image_url.split('\"')[1]
                    if image_url not in image_urls:
